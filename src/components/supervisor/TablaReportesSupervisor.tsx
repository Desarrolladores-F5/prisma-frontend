...
import { obtenerReportes } from '@/lib/api';

interface Props {
  refrescar?: boolean;
}

export default function TablaReportesSupervisor({ refrescar }: Props) {
  ...
  const reportesFiltrados = reportes.filter((r) => {
    ...
    const faenaMatch = filtroFaena === '' || r.faena?.nombre?.toLowerCase().includes(filtroFaena.toLowerCase());
    return tipoMatch && estadoMatch && faenaMatch;
  });

  return (
    <div className="mt-6">
      <h2 className="text-lg font-semibold mb-3">Reportes Registrados</h2>
