...
import { crearAuditoria, actualizarAuditoria, obtenerFaenas, obtenerUsuarios } from '@/lib/api';
...
export default function FormularioAuditoria({ auditoria, onGuardado }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded shadow-md mt-6 space-y-6">
      <h2 className="text-xl font-bold">{modoEdicion ? 'Editar Auditoría' : 'Registrar Auditoría'}</h2>

      {mensaje && <p className="text-sm text-blue-600 bg-blue-100 p-2 rounded border border-blue-300">{mensaje}</p>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="md:col-span-2">
          <label className="flex items-center space-x-2 text-sm">
            <input type="checkbox" name="conforme" checked={form.conforme} onChange={handleChange} />
